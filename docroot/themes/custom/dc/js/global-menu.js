...
(function (Drupal, $) {
   ...
   */
  Drupal.GlobalMenu = function ($links, options) {
    ...
    var searchToggle = function () {
      ...
      $('body').on('click touchstart', function (e) {
        ...
        if ($(e.target).closest('.block-globalsearch-block.open').length === 0 &&
          ...
          $('a.search-open').removeClass('search-open');
        }
      });
    };
  ...
  /**
   *
   * @type {{attach: Drupal.behaviors.GlobalMenu.attach}}
   */
  Drupal.behaviors.GlobalMenu = {
    attach: function (context, settings) {
      var $menuContainer = $('#block-dc-main-menu');

      $menuContainer.once('GlobalMenu').each(function () {
        var $links = $('.menu-level-0 > li > .link-container', $(this));
        if ($links.length > 0) {
          var menu = new Drupal.adcoMenu($links);
          menu.init();
        }
      });
    }
